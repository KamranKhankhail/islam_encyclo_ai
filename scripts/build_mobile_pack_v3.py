#!/usr/bin/env python3
"""
Build Mobile Pack v3 for React-Native (portable, integrity-checked).
...
Optional inputs (from output/processed/models):
- e5_query_encoder_xnnpack.pte
- e5_tokenizer.json
- e5_query_encoder_meta.json

Outputs (to output/mobile_pack/v3):
...
- pack_manifest_v3.json          (sha256 + size + counts)
- copies of: metadata.json, verse_keys_e5.json, embeddings_meta_e5.json
- optional: models/e5_query_encoder_xnnpack.pte
- optional: models/e5_tokenizer.json
- optional: models/e5_query_encoder_meta.json
...
# ----------------------------
# Mobile v3 configuration defaults
# ----------------------------
...
def resolve_paths(repo_root: Path, processed_dir: Optional[Path], out_dir: Optional[Path]) -> PackPaths:
    ...
    out = out_dir or (repo_root / "output" / "mobile_pack" / "v3")
...
def main():
    ...
    ap.add_argument("--out-dir", type=str, default=None, help="Override output/mobile_pack/v3 path")
    ap.add_argument("--pack-version", type=str, default="v3", help="Pack semantic version string")
    ...
    # Load metadata and optionally attach query encoder artifacts.
    metadata = read_json(paths.metadata)
    if not isinstance(metadata, dict):
        raise ValueError("metadata.json must be a JSON object")

    qe_model_src = paths.processed_dir / "models" / "e5_query_encoder_xnnpack.pte"
    qe_tokenizer_src = paths.processed_dir / "models" / "e5_tokenizer.json"
    qe_meta_src = paths.processed_dir / "models" / "e5_query_encoder_meta.json"

    include_query_encoder = qe_model_src.exists() and qe_tokenizer_src.exists()
    include_query_encoder_meta = False

    if include_query_encoder:
        models_out = paths.out_dir / "models"
        models_out.mkdir(parents=True, exist_ok=True)

        shutil.copy2(qe_model_src, models_out / qe_model_src.name)
        shutil.copy2(qe_tokenizer_src, models_out / qe_tokenizer_src.name)
        if qe_meta_src.exists():
            shutil.copy2(qe_meta_src, models_out / qe_meta_src.name)
            include_query_encoder_meta = True
        ...
        query_encoder_entry = {
            ...
            "dim": 384,
            "max_tokens": 128,
            "model_file": "models/e5_query_encoder_xnnpack.pte",
            "tokenizer_file": "models/e5_tokenizer.json",
        }
        if include_query_encoder_meta:
            query_encoder_entry["meta_file"] = "models/e5_query_encoder_meta.json"

        metadata["query_encoder"] = query_encoder_entry
    else:
        metadata.pop("query_encoder", None)
    ...
    if include_query_encoder:
        manifest_files.extend([
            "models/e5_query_encoder_xnnpack.pte",
            "models/e5_tokenizer.json",
        ])
        if include_query_encoder_meta:
            manifest_files.append("models/e5_query_encoder_meta.json")

    # Compute file hashes (manifest itself written last)
    ...
    pack_manifest = {
        ...
        "schema": "askquran_mobile_pack_v3",
    ...
    # Write manifest
    manifest_path = paths.out_dir / "pack_manifest_v3.json"
