#!/usr/bin/env python3
"""
Verify Mobile Pack v1 (hard fail on any mismatch).

Validates:
- Required files exist
- sha256(file) matches pack_manifest_v1.json for every listed file
- verse_embeddings_e5.f16.bin byte size == rows * dim * 2
- verses_mobile_v1.ndjson line count == verse_count
- verse_index_v1.json size == verse_count
- verse_index maps every verse_key to correct row index (alignment with verse_keys_e5.json)
- Optional: validate embeddings_meta sha256_array if processed .npy exists

Run:
  python scripts/verify_mobile_pack_v1.py
"""
...
def main() -> None:
    ...
    ap.add_argument("--pack-dir", type=str, default=None, help="Override pack dir (default: output/mobile_pack/v1)")
    ...
    pack_dir = Path(args.pack_dir) if args.pack_dir else (repo_root / "output" / "mobile_pack" / "v1")
    processed_dir = Path(args.processed_dir) if args.processed_dir else (repo_root / "output" / "processed")

    manifest_path = pack_dir / "pack_manifest_v1.json"
    if not manifest_path.exists():
        raise FileNotFoundError(f"Missing manifest: {manifest_path}")

    manifest = read_json(manifest_path)
    if not isinstance(manifest, dict):
        raise ValueError("pack_manifest_v1.json must be a JSON object")
